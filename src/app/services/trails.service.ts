    ...
    getTrailsByZip(zip: string, distance?: string): Observable<any> {
        return this.zipCodeService.getLocationDetails(zip).pipe(
            switchMap((locationDetails: ZipLocation) => {
                return this.apiService.get(`https://www.hikingproject.com/data/get-trails?lat=${locationDetails.latLng.lat}&lon=${locationDetails.latLng.lng}&maxDistance=${distance ? distance : '10'}&key=${this.keys.hikingAPI}&maxResults=1000`, undefined);
            })
        );
    }
}
